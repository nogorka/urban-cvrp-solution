...
def crossover(parent1, parent2, PID, DM):
    ...
    crossover_point = np.random.choice(np.arange(1, len(parent1) ), p=prob)
...
def inversion_mutation(tour):
    ...
    idx1, idx2 = sorted(random.sample(range(len(tour)), 2))

    # Perform the inversion
    new_tour = tour[:idx1] + tour[idx1:idx2][::-1] + tour[idx2:]
    return new_tour
...
def rgibnnm_mutation(route, distance_matrix):
    ...
    swap_with = random.choice([route[n] for n in neighbors if n != random_gene_idx])

    # Perform the swap mutation
    swap_with_idx = route.index(swap_with)
    route[random_gene_idx], route[swap_with_idx] = route[swap_with_idx], route[random_gene_idx]
...
def mutate(route, DM):
    intermediate_tour = inversion_mutation(route)
    return rgibnnm_mutation(intermediate_tour, DM)
...
def genetic_algorithm(population_size, generations, NP, PID, DM):
...
if __name__ == "__main__":
    ...
    best_route = genetic_algorithm(population_size=10, generations=300,
