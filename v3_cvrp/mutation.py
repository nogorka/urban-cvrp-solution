...
def swap_mutation(individual, capacity=None):
    ...
    if route.size > 1:
        i, j = np.random.choice(route.size, 2, replace=False)
...
def inter_route_swap_mutation(individual, capacity):
    ...
    if individual.size > 1:
        ...
        if r1.points and r2.points:
            i, j = choice(range(r1.size)), choice(range(r2.size))
...
def relocation_mutation(individual, capacity):
    ...
    if individual.size > 1:
        ...
        if source_route.points:
            point_index = randint(0, source_route.size - 1)
...
def inversion_mutation(individual, capacity=None):
    ...
    route = choice(individual.routes)
    if route.size > 1:
        start, end = sorted(np.random.choice(route.size, 2, replace=False))
